...
from nps_case.pipeline import load_dataset, missing_summary, run_pipeline


def test_load_dataset_normalizes_columns(tmp_path):
    dataset = tmp_path / "sample.csv"
    pd.DataFrame({"Nota NPS": [10, None], "Comentario Cliente": ["bom", "ruim"]}).to_csv(dataset, index=False)
    df = load_dataset(dataset)
    assert list(df.columns) == ["nota_nps", "comentario_cliente"]
...
def test_run_pipeline_writes_outputs(tmp_path):
    ...
    pd.DataFrame({"nota": [10, 5]}).to_csv(dataset, index=False)
